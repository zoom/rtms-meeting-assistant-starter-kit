// chatWithGemma.js - Google Gemma 3 27B IT integration via OpenRouter
...
/**
 * Sends a message to Google Gemma 3 27B IT via OpenRouter
 * @param {string} message - The user message
 * @param {string} model - The model to use (defaults to google/gemma-3-27b-it:free)
 * @returns {Promise<string>}
 */
export async function chatWithGemma(message, model = 'google/gemma-3-27b-it:free', images = []) {
  try {
    const content = [];
    ...
    console.error('❌ Error with Gemma:', err.response?.data || err.message);
    throw err;
  }
}

/**
 * Sends a message to Google Gemma 3 27B IT (fast version without images)
 * @param {string} message - The user message
 * @param {string} model - The model to use
 * @returns {Promise<string>}
 */
export async function chatWithGemmaFast(message, model = 'google/gemma-3-27b-it:free') {
  try {
    const response = await openai.chat.completions.create({
      model: model,
...
/**
 * Generate strategic dialog suggestions for meeting facilitation using Gemma
...
export async function generateDialogSuggestions(transcript) {
  try {
    ...
    console.log('🗣️ Generating dialog suggestions from transcript using Gemma...');
    const response = await openai.chat.completions.create({
    ...
    console.log(`✅ Generated ${suggestions.length} dialog suggestions with Gemma`);
    return suggestions.slice(0, 4); // Ensure max 4 suggestions
  } catch (err) {
    console.error('❌ Error generating dialog suggestions with Gemma:', err.message);
    return [
...
/**
 * Analyze sentiment from full meeting transcript for multiple users using Gemma
 * @param {string} transcript - Full meeting transcript text
...
export async function analyzeSentiment(transcript) {
  try {
    ...
    console.log('😊 Analyzing sentiment from full transcript using Gemma...');
    const response = await openai.chat.completions.create({
      model: 'google/gemma-3-27b-it:free',
      messages: [{ role: 'user', content: filledPrompt }],
    });
    ...
    try {
      ...
      console.log('✅ Sentiment analysis completed with Gemma:', Object.keys(sentimentData).length, 'users analyzed');
      return sentimentData;
    } catch (parseError) {
      console.error('❌ Error parsing sentiment JSON response from Gemma:', parseError.message);
      console.error('Raw response:', rawContent);
      // Return empty object as fallback
      return {};
    }
  } catch (err) {
    console.error('❌ Error analyzing sentiment with Gemma:', err.message);
    return {}; // Empty object as fallback
  }
}

/**
 * Generate a real-time meeting summary from transcript and images using Gemma
...
export async function generateRealTimeSummary(transcript, meetingEvents = '', imageBase64Array = [], streamId = '', meetingUuid = '') {
  try {
    ...
    console.log('📝 Generating real-time stream summary using Gemma...');
    const response = await chatWithGemma(filledPrompt, 'google/gemma-3-27b-it:free', imageBase64Array);

    console.log('✅ Real-time summary generated with Gemma');
    return response;
  } catch (err) {
    console.error('❌ Error generating real-time stream summary with Gemma:', err.message);
    return 'Unable to generate summary at this time. Meeting in progress...';
  }
}

/**
 * Query the current meeting transcript for specific questions using Gemma
...
export async function queryCurrentMeeting(transcript, userQuery) {
  try {
    ...
    console.log('🔍 Querying current meeting transcript using Gemma...');
    const response = await openai.chat.completions.create({
      model: 'google/gemma-3-27b-it:free',
      messages: [{ role: 'user', content: filledPrompt }],
    });
    ...
    console.log('✅ Meeting query answered with Gemma');
    return answer;
  } catch (err) {
    console.error('❌ Error querying current meeting with Gemma:', err.message);
